...
list_remove: (eq b) => (v: b) -> (l: list b) -> list b
...
listdiff:(eq a) => (l1:list a) -> (l2:list a) -> list a
listdiff l1 nil = l1
listdiff nil l2 = l2
listdiff (h::t) l2 = ite (member h l2)
                         (listdiff t (list_remove h l2))
                         (listdiff t l2)

-- Return the set difference, s1 minus s2
--set_difference: (eq a) => (s1: set a) -> (s2: set a) -> set a
set_difference (mkSet l1) (mkSet l2) = mkSet (listdiff l1 l2)
...
list_forall p (h::t) = ite (p h)
                         (list_forall p t)
                         false


set_forall p (mkSet l) = list_forall p l
...
-- Return true if p is true for some v in s, otherwise else false
-- set_exists: (p: a -> bool) -> (s: set a) -> bool
list_exists: (p: a -> bool) -> (s: list a) -> bool
list_exists p nil = false
list_exists p (h::t) = ite (p h)
                           true
                           (list_exists p t)

set_exists p (mkSet l) = list_exists p l
...
Truevalue:(p: a -> bool) -> list a -> a
Truevalue p (h::t) = ite (p h)
                       h
                       (Truevalue p t)

set_witness p (mkSet l) = ite (list_exists p l)
                       (some (Truevalue p l))
                       (none)
...
--pair elt takes an elt h and a list b and returns a list of all
...
set_Eql (mkSet s1) (mkSet s2) = sameElement s1 s2
